...
const HANDOFF_ID = "showcase-brand-product-handoff";
 ...
 */
export function useShowcaseBrandHandoff(refs: ShowcaseStageRefs) {
  useEffect(() => {
    const { showcaseSectionRef, showcaseProductImgRef, brandProductImgRef, brandSectionRef } =
      refs;
    ...
    if (prefersReducedMotion() || !section || !showcaseImg || !brandImg || !brandSection) {
      return;
    }

    document.documentElement.dataset.showcaseBrandHandoff = "1";

    const flyImg = document.createElement("img");
    flyImg.className = "showcase-to-brand-fly-img";
    flyImg.alt = "";
    ...
    function applyHandoff(self: ScrollTrigger) {
      ...
      const rF = showcaseImg!.getBoundingClientRect();
      const rB = brandImg!.getBoundingClientRect();

      if (rF.width < 4 || rB.width < 4) {
        flyImg.style.opacity = "0";
        showcaseImg!.style.removeProperty("opacity");
      ...
      const targetSrc = showcaseImg!.currentSrc || showcaseImg!.src || "";
      ...
      /* Mesmo instante: a Showcase some e o clone aparece — nunca os dois juntos. */
      ...
      if (p >= FLY_START) showcaseImg!.style.opacity = "0";
      else showcaseImg!.style.removeProperty("opacity");
    ...
    const trigger = ScrollTrigger.create({
      ...
      trigger: section,
      start: "top top",
      endTrigger: "#almeida-brand-product-parallax",
      end: "center center",
      scrub: 0.45,
    ...
    return () => {
      ...
      trigger.kill();
      flyImg.remove();
      delete document.documentElement.dataset.showcaseBrandHandoff;
      showcaseImg.style.removeProperty("opacity");
